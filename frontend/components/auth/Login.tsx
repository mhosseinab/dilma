...
import { getVerifyCode } from "redux/ActionCreators";

const AuthLogin: NextPage = () => {
  const [number, setNumber] = React.useState("");
  const { loginCode } = useSelector(
  ...
  const loginHandler = () => {
    if (number == "" || number.length !== 11) {
      toast.error("شماره موبایل مجاز نیست");
      return;
    } else {
      toast.success("کد برای شما ارسال خواهد شد");
      setTimeout(() => {
        dispatch(getVerifyCode(number));
      }, 1000);
    }
  };
